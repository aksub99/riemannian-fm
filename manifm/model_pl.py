...
class DualManifoldFMLitModule(pl.LightningModule):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        ...
        self.model_aa = EMA(
            Unbatch(  # Ensures vmap works.
                ProjectToTangent(  # Ensures we can just use Euclidean divergence.
                    tMLP(  # Vector field in the ambient space.
                        self.dim,
                        d_model=cfg.model_aa.d_model,
                        num_layers=cfg.model_aa.num_layers,
                        actfn=cfg.model_aa.actfn,
                        fourier=cfg.model_aa.get("fourier", None),
                    ),
                    manifold=self.manifold,
                    metric_normalize=self.cfg.model_aa.get("metric_normalize", False),
                )
            ),
            cfg.optim.ema_decay,
        )
        ...
        self.model_cg = EMA(
            Unbatch(  # Ensures vmap works.
                ProjectToTangent(  # Ensures we can just use Euclidean divergence.
                    tMLP(  # Vector field in the ambient space.
                        self.dim,
                        d_model=cfg.model_cg.d_model,
                        num_layers=cfg.model_cg.num_layers,
                        actfn=cfg.model_cg.actfn,
                        fourier=cfg.model_cg.get("fourier", None),
                    ),
                    manifold=self.manifold,
                    metric_normalize=self.cfg.model_cg.get("metric_normalize", False),
                )
            ),
            cfg.optim.ema_decay,
        )
    ...
    @property
    def vecfield_aa(self):
        return self.model_aa

    @property
    def vecfield_cg(self):
        return self.model_cg

    @property
    def device(self):
        return self.model_aa.parameters().__next__().device
    ...
    def optimizer_step(self, *args, **kwargs):
        super().optimizer_step(*args, **kwargs)
        if isinstance(self.model_cg, EMA):
            self.model_cg.update_ema()
        if isinstance(self.model_aa, EMA):
            self.model_aa.update_ema()
